...
Now, in order to run the algorithm, we just need to supply arguments to the `nBits` and `maxiter` parameters. Since we need 7-bits for each letter in the final result, we set `nBits` equal to `nchar(prose)*7`. Next, we set `maxiter` to 100,000 generations, a value which tended to achieve optimal results during prior testing.
...
Sometimes, the genetic algorithms result object includes many equally fit solutions. In these cases, I usually collapse all "best" solutions into an "average best" solution.
...
Finally, we join the characters together to draft .
