...
Using the same digital ribosome from above, we will convert [].

However, in order to ensure that a codon can become any and every letter, these codons will need to contain 7-bits of information each.

Then, we will set a [] of Shakespeare against which each random digital genome will get compared. Through these comparisons, we ensure that only the most prosaic randomness have the opportunity for reproduction.
...
In order to run the algorithm, we again need to supply arguments to the `nBits` and `maxiter` parameters. Since we need 7-bits for each letter in the final result, we set `nBits` equal to `nchar(prose)*7`.
