...
Which roles tend to follow other roles? Here, the number nearest the node indicates the outgoing probability.
...
Which roles tend to follow other roles? Here, rows indicate the first state (and columns the second).
...
For statistical modeling and visualization, we want the data in a neat format. Here, we convert a list of matrices into one large matrix. You can download these data [here](data/pr-frequency.csv).
...
We can also visualize how ownership of a building changes the rate of VP earnings (as further broken down by player and position). For example, we see below how ownership of the "Harbor" building yields more VP.
...
As an analyst and player, I want to know which buildings provide the most value in terms of VP earnings. One way to assess this is by fitting a linear model to VP earnings, and then comparing the weight of the coefficients for each building. By modeling turn order as a covariate (labeled as "X"), the coefficients of the linear model should estimate how well building ownership correlates with victory VP earnings while adjusting for the natural course of the game.
...
How does the value (in terms of VP earnings) of buildings change for each player?
...
How does the value (in terms of VP earnings) of buildings change for each player in first position?
...
How does the value (in terms of VP earnings) of buildings change for each player in second position?

```{r}
round(coefPerNP, 2)[,4:6]
```

How does the value (in terms of VP earnings) of buildings change for each player in third position?

```{r}
round(coefPerNP, 2)[,7:9]
```

The linear models above implicitly assume that the ownership of each building contributes independently to VP earned. However, we might expect that the joint ownership of some building pairs could contribute more toward VP earnings than the ownernship of either building individually. We can estimate the synergistic value of joint ownership by fitting another linear model that considers the interaction effect between building ownership. For this, we fit 529 linear models (i.e., for all pairs of 23 buildings), wherein we replace each pair of two-level factors (i.e., ownership of building A and B as binary states) with a four-level factor (i.e., ownership of neither, the first, the second, or both). As above, we include turn order as a covariate. However, because we fit a separate model for each building pair, we should fix the turn order coefficient (chosen as 0.58 from above) to keep results comparable across the 529 models.
...
Here, the coefficient "X11" represents the added value of joint ownership (i.e., the added value beyond owning each building individually "X10" and "X01"). As such, we can conceive of "X11" as a kind of synergy score which describes the amount of VP earned by owning both buildings that is not simply explained by owning both separately.
