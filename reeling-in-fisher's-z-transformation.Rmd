...
This transformation, known as the [Fisher z-transformation](https://en.wikipedia.org/wiki/Fisher_transformation), makes it possible to use correlation coefficients within a conventional hypothesis testing framework by providing a way to calculate standard error using only a point estimate (i.e., the sample correlation coefficient) and the sample size. In this way, it provides an elegant means by which to effortlessly apply well-described statistical methods to correlation-based studies.
...
Using this result, we can easily calculate both the empiric standard error and the predicted standard error for all $N$.
...
We compare these quantitatively by calculating the mean of the absolute difference between them.
...
Again, the Shapiro-Wilk normality test confirms that these z-transformed data follow a normal distribution.
